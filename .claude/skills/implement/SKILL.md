...
1. Resolve the branch `feat/<slug>` from the issue's `feat:<slug>` label, or from the plan slug when there is no issue. **Announce before touching git, then continue without waiting:** say either "resuming existing `feat/<slug>`" or "creating `feat/<slug>` off `<base>`", so the user can stop you and redirect where the work lands. Non-blocking: state it, do not halt. Then act: resume the branch if it exists; else create it off the base branch, resolved in this order: (a) the issue's recorded base (its `## Branch` line), (b) the current branch when there is no issue. Do not assume `main`: `git checkout <base>` (pull if it tracks a remote), then `git checkout -b feat/<slug>`. Every issue of a feature shares the key, so all of them land on one branch and later issues resume it. No memory needed.
