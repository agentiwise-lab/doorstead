...
## The PR
One pull request, `feat/listing-image-uploads` into `04_end`, carrying AGE-115 to AGE-119 (validation, variants, publish guard, order/cover/floorplan, one render path). AGE-120 (destructive upload UI) is held at the issue level and was never in the PR.
...
## Merge
Squash-merged, one clean revertable commit on the trunk. The trunk re-verifies green from a clean checkout. AGE-120 remains held; the two minors ride as tracked follow-ups.
