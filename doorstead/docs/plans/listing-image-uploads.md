...
- Modules + contracts:
  - `lib/db/storage.ts` (new; only caller of Supabase Storage): `uploadObject(key: string, bytes: Uint8Array, contentType: string): Promise<void>` (uses `createServerClient()` so the admin-write RLS passes), `createSignedUrl(key: string, expiresInSeconds: number): Promise<string>` (uses the anon client, permitted by the scoped SELECT below). Client is chosen per method here, the way `DefaultListingService` already picks its client per operation; a single shared client cannot serve both trust contexts.
  ...
  - `lib/listings/contract.ts`: extend read model so a listing exposes resolved image links for render, e.g. `getImagesForRender(listingId: string): Promise<RenderImage[]>` where `RenderImage = { url: string; isFloorplan: boolean }` (signed url for stored media; passthrough url for legacy). Ownership fixed here: this method lives on `ListingService`'s contract, and `DefaultListingService` depends on the `MediaService` **contract** (injected), never its impl, so tests drive it through `FakeMediaService` and the one-directional `service -> service contract` rule in `CLAUDE.md` holds. Units 3/5/6 extend this same method, so the ownership is settled now, not re-litigated later
  - migration `supabase/migrations/0003_listing_media.sql`: `listing_media` table (listing_id, original_key, position, is_cover, is_floorplan, timestamps); the private bucket provisioned in SQL via `insert into storage.buckets` (id `listing-media`, `public=false`); RLS on `storage.objects` for this bucket: the admin (server client) may write, and the anon role may SELECT only objects that belong to a live listing (an `exists` check joining the object key to `listing_media` -> `listings.status = 'live'`). That scoped anon SELECT is exactly what lets the anon-key public page mint a signed link, while draft-listing and unrelated objects stay unreadable and unenumerable. All under `npx supabase db push`, nothing created out of band
